...
function updateAuthUI() {
  ...
  if (state.user) {
    area.innerHTML = `
      <span>Hi, ${state.user.username}</span>
...
async function loadProducts() {
  ...
  try {
    ...
    $('#product-grid').innerHTML = `<p class="error">${err.message}</p>`;
  }
}
...
function renderProducts() {
  ...
  if (!state.products.length) {
    grid.innerHTML = '<p class="empty-state">暂无商品</p>';
    return;
  }
...
function renderCart() {
  ...
  if (!state.cart.items.length) {
    el.innerHTML = '<p class="empty-state">购物车是空的，去逛逛吧</p>';
    return;
  }
  ...
  `).join('') + `
    <div class="cart-footer">
      <div class="cart-total">合计: <span>¥${state.cart.total.toFixed(2)}</span></div>
      <button class="btn btn-primary" id="checkout-btn">去结算</button>
    </div>
  `;
  ...
  el.querySelectorAll('[data-qty-inc]').forEach((btn) => {
    btn.onclick = () => updateQty(btn.dataset.qtyInc, 1);
  });
  el.querySelectorAll('[data-qty-dec]').forEach((btn) => {
    btn.onclick = () => updateQty(btn.dataset.qtyDec, -1);
  });
}
...
function openCheckout() {
  $('#checkout-summary').innerHTML = `
    <p>共 ${state.cart.items.length} 件商品</p>
    <p><strong>合计: ¥${state.cart.total.toFixed(2)}</strong></p>
    <p style="margin-top:0.5rem;font-size:0.8rem;color:#6b6b6b">
      提交后订单将通过 RocketMQ 异步处理（扣库存 → 支付 → 发货）
    </p>
  `;
...
async function handleCheckout(e) {
  ...
  try {
    const data = await api('/orders/checkout', {
      method: 'POST',
      body: JSON.stringify({ shippingAddress: fd.get('shippingAddress') }),
    });
...
async function loadOrders() {
  ...
  try {
    ...
    if (!orders.length) {
      el.innerHTML = '<p class="empty-state">暂无订单</p>';
      return;
    }
  ...
  return `
    <div class="order-card" data-order-id="${o.id}">
      ...
      <div class="order-meta">
        <p>金额: ¥${Number(o.total_amount).toFixed(2)}</p>
        <p>地址: ${o.shipping_address}</p>
        <p>下单: ${new Date(o.created_at).toLocaleString('zh-CN')}</p>
      </div>
    </div>
  `;
}
  ...
  pollTimer = setInterval(async () => {
    if ($('#page-orders').classList.contains('active')) {
      try {
        ...
        $('#orders-list').innerHTML = orders.map((o) => renderOrderCard(o)).join('');
        if (!hasPending) clearInterval(pollTimer);
      } catch { /* ignore */ }
    }
  }, 3000);
}
  ...
  try {
    ...
    $('#admin-orders').innerHTML = `
      <table class="admin-table">
        <thead><tr>
          <th>订单号</th><th>用户</th><th>金额</th><th>状态</th><th>时间</th><th>操作</th>
        </tr></thead>
        <tbody>${orders.map((o) => `
          <tr>
            <td>#${o.id}</td>
            ...
            <td>
              ${o.status === 'shipped' ? `<button class="btn btn-sm" data-complete="${o.id}">完成</button>` : ''}
              ${['pending', 'processing'].includes(o.status) ? `<button class="btn btn-sm btn-danger" data-cancel="${o.id}">取消</button>` : ''}
            </td>
          </tr>
        `).join('')}</tbody>
      </table>
    ...
    $('#admin-orders').querySelectorAll('[data-complete]').forEach((btn) => {
      btn.onclick = () => updateOrderStatus(btn.dataset.complete, 'completed');
    });
    $('#admin-orders').querySelectorAll('[data-cancel]').forEach((btn) => {
      btn.onclick = () => updateOrderStatus(btn.dataset.cancel, 'cancelled');
    });
    ...
    $('#admin-orders').innerHTML = `<p class="error">${err.message}</p>`;
  }
}
